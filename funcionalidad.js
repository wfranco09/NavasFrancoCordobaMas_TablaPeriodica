...
function cargarVistaMovil() {
  fetch('elementosmovil.json')
    .then(res => res.json())
    .then(data => {
      const tabla = document.querySelector('.tabla-grid')
      tabla.innerHTML = '' // Limpiar
      ...
      data.secciones.forEach(seccion => {
        ...
        // Crear grid de elementos con máximo 2 columnas
...
function determinarVista() {
  ...
  if (ancho <= 600) {
    // Para dispositivos hasta 600px: TODAS las secciones separadas con máximo 2 columnas
    cargarVistaMovil()
  } else if (ancho <= 800) {
    // Para dispositivos 601px-800px: secciones agrupadas como estaba definido
    cargarVistaResponsive()
  } else {
    // Para dispositivos más grandes usar vista desktop (grid tradicional)
    cargarVistaDesktop()
  }
}
